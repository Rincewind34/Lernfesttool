...
public class StudentList implements CustomPrintable {
	...
	public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {
		...
		for (; index < this.students.size(); index = index + 1) {
			...
			String strSchoolClass = "NaN";
			String strRoom = "NaN";

			if (student.isSchoolClassSelected()) {
				SchoolClass schoolClass = student.getValue(Student.SCHOOL_CLASS);
				strSchoolClass = schoolClass.toString();

				if (schoolClass.isRoomSelected()) {
					strRoom = schoolClass.getValue(SchoolClass.ROOM).toString();
				}
			}

			this.drawRow(graphics, pageFormat, 1, currentY);
			this.fillRow(graphics, currentY, strSchoolClass, strRoom, student.getValue(Student.LASTNAME) + ", " + student.getValue(Student.FIRSTNAME));
	...
	private void drawRow(Graphics graphics, PageFormat pageFormat, double height, double y) {
		graphics.drawLine(0, Design.centiToPoints(y), (int) pageFormat.getImageableWidth(), Design.centiToPoints(y));
		...
		graphics.drawLine((int) pageFormat.getImageableWidth(), Design.centiToPoints(y), (int) pageFormat.getImageableWidth(),
